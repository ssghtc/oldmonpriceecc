...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        title: `${post?.title || 'Post'} - Top Software`,
        description: post?.description,
    };
}

export default async function SoftwarePostPage({ params }: Props) {
    const { slug } = await params;
    const categoryId = 'software';
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            <div className="bg-zinc-900/40 border border-amber-900/10 rounded-2xl p-8 backdrop-blur-sm">
                                <h2 className="text-2xl font-bold text-amber-500 mb-4">Software Overview</h2>
                            ...
                            <div className="bg-zinc-900/40 border border-amber-900/10 rounded-2xl p-8 backdrop-blur-sm">
                                <h2 className="text-2xl font-bold text-amber-500 mb-6">Capabilities</h2>
                        ...
                        <div className="space-y-6">
                            <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-6 sticky top-24">
                                <h3 className="text-xl font-bold text-amber-500 mb-4">Download Hub</h3>
                                <a
                                ...
                                <p className="text-zinc-500 text-sm mt-4 text-center">
                                    Trust official sources for the latest updates.
                                </p>
                            </div>
                        </div>
                    </div>
